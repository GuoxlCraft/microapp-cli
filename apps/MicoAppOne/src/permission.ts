...
router.beforeEach(async (to: Route, from: Route, next: any) => {
  // 获取当前环境的地址配置
  if (wsCache.get(appStore.userInfo) || isMicroApp) {
    if (to.path === '/login') {
      next({ path: '/' })
    } else {
      if (!isMicroApp && !dictStore.isSetDict && !isMicroApp) {
...
function recursionRoutes(routes, basePath = '/') {
  ...
  for (const route of routes) {
    ...
    if (
      ...
      if (!route.tagName) {
        route.tagName = route.code
      }
      if (route.children?.length) {
        for (const v of route.children) {
          v.noTagsView = true
          v.hidden = true
          v.activeMenu = '/' + route.url
          v.tagName = route.tagName
          res.push(v)
        }
